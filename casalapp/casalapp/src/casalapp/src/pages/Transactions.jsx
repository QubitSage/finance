...
export default function Transactions() {
  ...
  const handleAdd = async (e) => {
    e.preventDefault()
    await insert({ ...form, amount: parseFloat(form.amount), type: tab === 'income' ? 'income' : 'expense', month })
  ...
  return (
    <div className="p-4 md:p-6 max-w-3xl mx-auto">
      <PageHeader title="Entradas & Saídas"
      ...
      <div className="grid grid-cols-2 gap-3 mb-4">
        ...
        <div className="card"><p className="stat-label">Saídas</p><p className="text-xl font-display font-semibold text-blush-500">{fmt(totalExpense)}</p></div>
      </div>
      ...
      <div className="flex bg-stone-100 rounded-xl p-1 mb-4">
        {[['income','Entradas'],['expense','Saídas']].map(([v,l]) => (
          <button key={v} onClick={() => { setTab(v); setAdding(false) }}
            className={`flex-1 py-2 text-sm font-medium rounded-lg transition-all ${tab === v ? 'bg-white text-stone-800 shadow-warm' : 'text-stone-400 hover:text-stone-600'}`}>
            {l}
          </button>
        ))}
      </div>
      ...
      {adding && (
        <form onSubmit={handleAdd} className="card mb-4">
          <p className="form-section-title">Nova {tab === 'income' ? 'entrada' : 'saída'}</p>
          <div className="grid gap-3">
            <div><label className="label">Descrição</label><input className="input" value={form.description} onChange={e => setForm(p => ({...p, description: e.target.value}))} required placeholder="Ex: Guilherme cliente" /></div>
      ...
      <div className="tbl-wrap">
        <div className="overflow-x-auto">
          <table className="tbl">
            <thead><tr><th>Descrição</th><th>Categoria</th><th className="text-right">Valor</th><th className="w-10"></th></tr></thead>
            <tbody>
              {list.length === 0 ? (
