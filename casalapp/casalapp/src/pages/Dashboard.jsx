...
import { ChevronLeft, ChevronRight, TrendingUp, TrendingDown, Wallet, Heart, Sparkles, Home, Gem, Briefcase } from 'lucide-react'
...
export default function Dashboard() {
  ...
  const stats = [
    ...
    { label: 'Saidas', value: fmt(expense), icon: TrendingDown, color: 'text-blush-500', bg: 'bg-blush-50' },
    ...
    { label: (settings?.wife_percentage || 30) + '% Mimos Esposa', value: fmt(wifeShare), icon: Heart, color: 'text-pink-500', bg: 'bg-pink-50' },
    { label: (settings?.apartment_percentage || 40) + '% Apartamento', value: fmt(apartmentShare), icon: Home, color: 'text-amber-600', bg: 'bg-amber-50' },
  ...
  return (
        <div className="p-4 md:p-6 max-w-4xl mx-auto">
              ...
              <div className="flex items-center gap-3 mb-5">
                      <button onClick={prevMonth} className="btn-icon"><ChevronLeft className="w-4 h-4" /></button>button>
                      <span className="text-sm font-medium text-stone-600 min-w-[140px] text-center">{monthLabel(month)}</span>span>
                      <button onClick={nextMonth} className="btn-icon"><ChevronRight className="w-4 h-4" /></button>button>
              </div>div>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
                {stats.map(({ label, value, icon: Icon, color, bg }) => (
                    <div key={label} className="card">
                                ...
                                <p className="stat-label">{label}</p>p>
                                <p className="text-xl font-semibold font-display text-stone-800">{value}</p>p>
                    </div>div>
                  ))}
              </div>div>
          {pieData.length > 0 ? (
                  <div className="card">
                            <h2 className="font-medium text-stone-700 text-sm mb-4">Saidas por categoria</h2>h2>
                            <div className="flex flex-col md:flex-row items-center gap-6">
                                    ...
                                    <div key={d.name} className="flex items-center gap-2 text-sm">
                                                      ...
                                                      <span className="text-stone-500 flex-1">{d.name}</span>span>
                                                      <span className="font-medium text-stone-700">{fmt(d.value)}</span>span>
                                    </div>div>
                                  ))}
                                        </div>div>
                            </div>div>
                  ...
                  <div className="card text-center py-10">
                            <p className="text-stone-300 text-sm">Nenhuma transacao este mes ainda.</p>p>
                  </div>div>
              )}
        </div>div>
      )
