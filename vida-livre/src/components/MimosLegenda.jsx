...
export default function MimosLegenda() {
  ...
  return (
    <div className="rounded-2xl border border-[var(--color-vl-border)] bg-[var(--color-vl-elevated)]/50">
      <button
        type="button"
        onClick={() => setOpen((o) => !o)}
        className="flex w-full items-center justify-between gap-2 px-4 py-3 text-left"
      >
        <span className="flex items-center gap-2 text-sm font-semibold text-fuchsia-200">
          <Info size={15} className="text-fuchsia-400" />
          Legenda
        </span>
      ...
      {open && (
        <div className="space-y-3 border-t border-[var(--color-vl-border)] px-4 pb-4 pt-3">
          {Object.entries(MIMO_CONTEXTO).map(([key, cfg]) => (
            <div key={key} className={`rounded-xl border p-3 ${cfg.className}`}>
